...
<head>
    ...
    <title>Gestion des montures</title>
</head>
<body class="bg-black">
    ...
    <div id="success-message-edit" class="mb-6 p-4 bg-green-500/10 border border-green-500 text-green-500 rounded-lg flex items-center gap-3 animate-pulse">
        ...
        <span class="text-sm font-bold uppercase">Monture mise à jour avec succès !</span>
    </div>
<?php endif; ?>

<?php if (isset($_GET['success']) && $_GET['success'] === 'mount_deleted'): ?>
    <div id="success-message-user" class="mb-6 p-4 bg-green-500/10 border border-green-500 text-green-500 rounded-lg flex items-center gap-3 animate-pulse">
        <i class="ph ph-check-circle text-2xl"></i>
        <span class="text-sm font-bold uppercase">Monture supprimée avec succès !</span>
    </div>
<?php endif; ?>

<?php if (isset($_GET['success']) && $_GET['success'] === 'mount_added'): ?>
    <div id="success-message-user-added" class="mb-6 p-4 bg-green-500/10 border border-green-500 text-green-500 rounded-lg flex items-center gap-3 animate-pulse">
        <i class="ph ph-check-circle text-2xl"></i>
        <span class="text-sm font-bold uppercase">Monture créée avec succès !</span>
    </div>
<?php endif; ?>
...
<?php endif; ?>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-4 mb-6 bg-[#1a0f0a] p-3 border-t border-b border-primary-orange">
                <div class="xl:col-span-6 relative">
                    ...
                    <input type="text" id="search-mount" placeholder="Rechercher une monture" class="w-full bg-black/40 border border-amber-900/70 rounded-md py-2 pl-10 pr-4 text-sm focus:outline-none focus:border-primary-orange text-amber-100">
                </div>
                    ...
                    <?php foreach ($all_mounts as $mountRow) : ?>
                        <div role="row" data-name="<?php echo htmlspecialchars(strtolower($mountRow['name'])); ?>" class="mount-row flex flex-col xl:grid xl:grid-cols-12 bg-row-dark xl:items-stretch border-b border-primary-orange gap-3 xl:gap-0 p-5 xl:p-0 hover:bg-amber-500/5 transition-all group">
                            ...
                            <div role="cell" class="xl:col-span-2 xl:p-4 flex justify-end xl:justify-center items-center gap-3">
                                <a href="view_mount.php?id=<?php echo $mountRow['id']; ?>" class="p-2 border border-primary-orange rounded text-primary-orange hover:bg-primary-orange hover:text-black transition-all" title="Voir">
                                    <i class="ph ph-eye text-2xl"></i>
                                </a>
                                <a href="edit_mount.php?id=<?php echo $mountRow['id']; ?>" class="p-2 border border-primary-orange rounded text-primary-orange hover:bg-primary-orange hover:text-black transition-all" title="Modifier">
