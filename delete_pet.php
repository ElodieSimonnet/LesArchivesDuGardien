...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: admin_pet_gestion.php?error=invalid_method');
    exit;
}
...
if ($pet_id < 0) {
    header('Location: admin_pet_gestion.php?error=invalid_id');
    exit;
}
...
try {
    ...
    header('Location: admin_pet_gestion.php?success=pet_deleted');
    exit;
