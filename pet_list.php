                ...
                <?php
                    foreach ($pets as $pet) {
                        ...
                        <div class="pet-item relative flex flex-col">
                            <button class="wishlist-btn group absolute top-4 right-4 z-10 '.($pet['is_wishlisted'] ? 'is-favorite' : '').'"'.(isset($_SESSION['user_id']) ? ' data-type="pet" data-id="'.$pet['id'].'" data-csrf="'.$_SESSION['csrf_token'].'"' : '').' aria-label="Ajouter '.$eName.' aux favoris">
                            ...
                            <article data-owned="'.$statusValue.'"
                                    ...
                                    class="pet-card w-full h-full bg-primary-black border-2 border-primary-orange rounded-xl overflow-hidden flex flex-col transition-all duration-300 group shadow-2xl cursor-pointer '.($pet['is_owned'] ? '' : 'sepia hover:sepia-0').'">
