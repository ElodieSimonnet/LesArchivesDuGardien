                    ...
                    <?php foreach ($all_news as $i => $newsRow) : ?>
                        <div role="row" data-name="<?= htmlspecialchars(strtolower($newsRow['title'])) ?>" class="news-row flex flex-col xl:grid xl:grid-cols-12 <?= $i % 2 === 0 ? 'bg-row-dark' : 'bg-row-light' ?> xl:items-stretch border-b border-primary-orange gap-3 xl:gap-0 p-5 xl:p-0 hover:bg-amber-500/5 transition-all group">
                            ...
                            <div role="cell" class="xl:col-span-1 xl:p-4 flex justify-between xl:justify-center items-center xl:border-r xl:border-primary-orange border-dashed">
                                ...
                                <span class="font-bold text-primary-white">#<?= htmlspecialchars($newsRow['id']) ?></span>
                            </div>
                            ...
                            <div role="cell" class="xl:col-span-4 xl:p-4 flex justify-between xl:justify-center items-center xl:border-r xl:border-primary-orange border-dashed">
                                ...
                                <span class="font-bold text-primary-white text-sm"><?= htmlspecialchars($newsRow['title']) ?></span>
                            </div>
                            ...
                            <div role="cell" class="xl:col-span-2 xl:p-4 flex justify-between xl:justify-center items-center xl:border-r xl:border-primary-orange border-dashed">
                                ...
                                <span class="text-sm text-primary-white font-black uppercase"><?= htmlspecialchars($newsRow['author']) ?></span>
                            </div>
                            ...
                            <div role="cell" class="xl:col-span-3 xl:p-4 flex justify-between xl:justify-center items-center xl:border-r xl:border-primary-orange border-dashed">
                                ...
                                <span class="text-sm text-primary-white text-center">
                                    <time datetime="<?= date('Y-m-d', strtotime($newsRow['created_at'])) ?>"><?= date('d/m/Y H:i', strtotime($newsRow['created_at'])) ?></time>
                                </span>
                            </div>

                            <div role="cell" class="xl:col-span-2 xl:p-4 flex justify-end xl:justify-center items-center gap-3">
                                <a href="view_news.php?id=<?= $newsRow['id'] ?>" class="p-2 border border-primary-orange rounded text-primary-orange hover:bg-primary-orange hover:text-black transition-all" title="Voir" aria-label="Voir <?= htmlspecialchars($newsRow['title']) ?>">
                                    <i class="ph ph-eye text-2xl" aria-hidden="true"></i>
                                </a>
                                <a href="edit_news.php?id=<?= $newsRow['id'] ?>" class="p-2 border border-primary-orange rounded text-primary-orange hover:bg-primary-orange hover:text-black transition-all" title="Modifier" aria-label="Modifier <?= htmlspecialchars($newsRow['title']) ?>">
                                    <i class="ph ph-pencil-simple text-2xl" aria-hidden="true"></i>
                                </a>
                                <form action="delete_news.php" method="POST" class="delete-form inline">
                                    <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                                    <input type="hidden" name="news_id" value="<?= $newsRow['id'] ?>">
                                    <button type="submit" class="p-2 border border-primary-orange rounded text-primary-orange hover:bg-red-600 transition-all hover:border-none hover:text-black cursor-pointer" title="Supprimer" aria-label="Supprimer <?= htmlspecialchars($newsRow['title']) ?>">
